...
you specify no rules at all (or if you don't even have a makefile).  After
every makefile is loaded, makepp also loads a set of default rules.  (These
...
Makepp supplies default values for a number of variables.  These values
will be overridden by assigning to these variables in your makefile.

=over 4

=item C<$(CC)>

The C compiler.  Makepp looks for gcc, cc, or some other variants on your
system and tries to pick an appropriate compiler.

=item C<$(CFLAGS)>

C<-g> unless you're using the GNU compiler, in which case it's S<C<-g -Wall>>.

=item C<$(CXX)>

The C++ compiler.  Makepp looks for g++, c++, cxx, or CC
...
S<C<yacc> or C<bison -y>>, depending on what you have on your system.

=item C<$(LEX)>

C<lex> or C<flex>, depending on what you have on your system.

=back

If you have any questions about what any variable evaluates to, you can
always put a line in your makefile like this:

    dummy := $(print $(CC))

which simply prints the value of the C<$(CC)> variable when the makefile
...
C<infer_objects> attempts to infer what other C<.o> files need to be linked in
based on what C<.h> files are included; see L<makepp_functions/infer_objects>
for details.
