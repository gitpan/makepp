...
=for vc $Id: makepp_faq.pod,v 1.13 2013/02/16 15:11:37 pfeiffer Exp $
...
Makepp is part of Gentoo Linux.  Note that their 2.0-r1 is not the release
candidate 1, but the final released version 2.0.
...
Perl scripts so they are architecture independent.
...
Where the HTML documentation goes (default: I<datadir>/html), or 'none' if you
do not want it installed.

=item -m, --mandir=/F<path/to/man>

Where the manual pages should reside (default: I<prefix>/man), or 'none' if
you do not want them installed.
...
The customary file F<Makefile.PL> is only present for technical reasons.  It
will B<not> help you to install.
