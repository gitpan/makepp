...
=for vc $Id: makepp_command.pod,v 1.53 2011/10/30 20:57:44 pfeiffer Exp $
...
B<O:>E<nbsp>L<-o|/o_filename>,
  ...
  L<--out-of-sandbox|/out_of_sandbox_filename>,E<nbsp>
...
statement.  Possible values are C<md5> and C<C> or C<c_compilation_md5>.
However, this option has no effect on the signature method for C/C++
compilation; you must use the C<signature> statement or the C<:signature> rule
modifier to affect that.  For more details, see L<makepp_signatures>.
