...
=for vc $Id: makepp_scanning.pod,v 1.16 2010/10/18 21:40:21 pfeiffer Exp $
...
Given a rule
    ...
    foo.o:
 	time libtool cc -c -I somewhere foo.c

makepp knows that C<time> and C<libtool> must be skipped and that C<cc> is the
actual command to be parsed here.  It understands that F<a.c> is the input
file and thus a dependency of this rule.  Moreover it will scan that file
looking for include statements, also in directory F<somewhere>.
...
must return an object of type C<Mpp::Lexer>, which will be used to
...
C<Mpp::Lexer> base class is used.
That object determines the scanner based on the first word of each command.
The base name is sought as a key in %parsers hash in the makefile's package,
...
This method is called automatically when a value from the %parsers hash does
