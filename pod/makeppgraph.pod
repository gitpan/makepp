...
=for vc $Id: makeppgraph.pod,v 1.7 2007/02/20 22:34:47 pfeiffer Exp $
...
are various selection, renaming and merging possibilities in C<makeppgraph>
described under L<Options|options>.
...
default unless C<--includes> is also given.
...
Instead of dependencies (or with C<--dependencies> additionally to them) draw
a graph of include relationships.  This will only have been logged as far as
it needed to be analyzed.  To get the full picture you need a fresh full
build.
...
Perform I<perlcode> for every target and its dependencies.  This API is still
under development!  Currently the target is passed in C<$_> and the dependency
as an argument.  If perlcode returns a value, that value replaces both the
target and the dependency, merging them into one node.  A few predefined
...
This is the first name rewriting that occurs.  For every name encountered,
perlcode gets called.  It gets a filename in C<$_>, and it may modify it.
This is often needed, bacause makepp logs fully qualified file names, so one
node can easily be half a screen wide.
