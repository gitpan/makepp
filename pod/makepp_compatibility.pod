...
=for vc $Id: makepp_compatibility.pod,v 1.14 2008/05/17 14:54:45 pfeiffer Exp $
...
require statement.  Strangely this only happens in one test, so you are
...
Winodws programs can handle normal slashes as directory separators, this does
...
It cannot do smart B<recursive> makes (but who would want them, since they are
known to be a broken paradigm) and B<parallel> builds.  Perl 5.6.0 fails on
quite a few tests, especially with $SHELL, 5.6.1 fails on two tests.  The test
suite, if it is running on 5.8.6 or lower, will rewrite FileInfo.pm so as to
have a needed workaround for an lstat bug.
