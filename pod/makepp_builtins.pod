...
=for vc $Id: makepp_builtins.pod,v 1.32 2007/04/24 22:55:53 pfeiffer Exp $
...
This allows reading arguments (options or plain args) from the given file, one
per line.  The arguments read replace this option.
...
length I<number> rather than line by line.
...
--inpipe=shellcommand, -I, --infail, -o, --output=filename, -O, --outfail, -v,
--verbose>
...
Copy I<sourcefile> to I<destfile>, or multiple I<sourcefile>s to I<destdir>
with the same name.
...
--characters=list>.  This is definitely inefficient for big files, if you have
...
Standard options: C<-A, --args-file, --arguments-file=filename, -f, --force, -i,
--inpipe=shellcommand, -I, --infail, -o, --output=filename, -O, --outfail, -r,
--record-size=number, -s, --separator=string, --verbose>

Only C<&grep> has extra options:
...
true.  On the other hand the C<--lines> option takes a Perl list expression
for forming a list slice, and thus starts at 0.  Complain to Larry :-(
...
Link I<sourcefile> to I<destfile>, or multiple I<sourcefile>s to I<destdir>
with the same name.
...
line.  Due to the rather simple parser these are not always performed before
invocations on the same line.  Therefore it is recommended to put them on a
line of their own -- followed by a backslash if you don't want an empty line.

You can also call C<@{ I<Perlcode> }@> or C<@@{ I<Perlcode> }@@ ... @@> in the
file.  The Perl variable C<$ARGV> contains the name of the current input file.
If you call C<@macro { I<Perlcode> }@>, then you define a new macro, the body
of which is a perl sub.  The arguments, if there are any, get passed in as
C<@_>.
