...
current directory or the system include directory (C</usr/include>).  This
...
C</home/users/joe/qt>, which means that the include files are stored in the
directory C</home/users/joe/qt/include>.  In your code, you want to be able to
do things like this:
...
There is no usually space between the C<-I> and the directory name.

When the C++ compiler is looking for the file C<qwidget.h>, it will look in
C</home/users/joe/qt/include> before looking in the system include directory.
...
<b>Beware:</b> if you specify more than one C<-l> option, the order can make a
...
system libraries.  C<-labc> searches for a file called C<libabc.a> or
C<libabc.so> or C<libabc.sa> in the system library directories (C</usr/lib>
...
libraries in C</home/users/joe/qt>, which means that the library files are in
C</home/users/joe/qt/lib>.  Your link step for your program might look
something like this:
