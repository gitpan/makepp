...
=for vc $Id: makeppinfo.pod,v 1.11 2011/10/30 20:57:44 pfeiffer Exp $
...
key.  The C<md5> signature in C<MD5_SUM>, the smart C<C> or C<c_compilation_md5>
...
If you want only filenames (useful with C<-t|--traverse>) select an unknown
key like "none".?????
...
order.  If this is the case for you, use C<diff -b> and/or sort the outputs.
The latter messes them up, but since each command is different the output
should remain technically equivalent after sorting.
