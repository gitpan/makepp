...
=for vc $Id: makepp_repositories.pod,v 1.17 2008/06/01 21:54:04 pfeiffer Exp $
...
and never updates its list, except for files it expects to appear.
...
The best current technique (alas not for user root) for this problem is to
make the file that you want not to include in the build process unreadable,
like this:
