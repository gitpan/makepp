...
=for vc $Id: makepp_repositories.pod,v 1.16 2006/11/07 22:44:58 pfeiffer Exp $
...
L<makepp_command/-F> for details.  You must specify C<-R> before C<-F>.)
...
and, bacause options are seen before makefiles are read, you can call just

    $ makepp -R/path/to/group/repository

=head2 Not respecifying your repository

The first time you must call makepp at the root of your local build tree with
a C<-R> option.  Thereafter you can use the fact that the repository links
remain from call to call.  But this alone is often not enough, because only
the individual files remember where they last came from.  If you add more
files or if the files become present in a nearer repository, that would not be
noticed.  You can however use the fact that the makefile remains, to have a
C<repository> statement:

=over

=item *

Add the following lines to the beginning of your F<RootMakeppfile> (preferably
by that name, because that is guaranteed to be loaded first):

    REPOSITORY ?= /path/to/your/repository
    ifeq $(relative-filename $(REPOSITORY)),.
      REPOSITORY =
    else
...
The previous mechanism can also be implemented locally (presumably outside of
your version control).  Put a little dummy F<RootMakeppfile> into your local
root directory, with only these lines:

    REPOSITORY ?= /path/to/your/repository
    repository $(REPOSITORY)
    include $(REPOSITORY)/RootMakeppfile # Pull in the real content.
...
=head3 Don't build in a repository

A repository is meant to be read-only while it is being used as a
repository.  Makepp will B<not> work properly if you change files in
your repository during the course of a build.  Before it starts the
build, makepp gets a list of all the files that exist in the repository,
and never updates its list.
...
For example, consider this command from the KDE distribution:
    ...
    libkdecore.la.closure: $(libkdecore_la_OBJECTS) $(libkdecore_la_DEPENDENCIES)
    	&echo "int main() {return 0;}" -o libkdecore_la_closure.cpp
...
directory:
    ...
    ../include/%.h : include/%.h
    	&cp $(input) $(output)
    # You could also (more efficiently) do the following:
    #    &ln -r $(input) $(output)

Similarly, if a repository also itself has a second repository, then files
in the second repository are I<not> linked into the first repository in order
to link it into the local directory.
If this behavior is required, then you need to set the second repository as
a repository for the local directory as well, so that it can link linked into
the local directory directly from the second repository.

Actually it's not quite that simple, because makepp might still try to build
files that happen to be in a repository if something asks for them
directly, but it won't build them I<on behalf> of the local directory.
The result of this can be quite confusing, because it can lead to a repository
symbolic link being used while its repository target is out-of-date, but that
target might get updated later in the build.
You can prevent this from happening either by making sure that the repository
is referred to I<only> through the repository path, or by making sure that
