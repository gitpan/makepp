...
=for vc $Id: makepp_rules.pod,v 1.22 2007/05/06 10:42:41 pfeiffer Exp $
...
Uses the scanner for C/C++ compilation, which looks for include files.

=item esqlc_compilation

Uses the scanner for Embedded SQL in C/C++ (pre)compilation, which looks for
include files.  If you use Oracle Pro*C, it also understands the config
parameter, parsing the file and also depending on it.  So, if you have
C<config=whatever.oracfg>, you can give a more readable variant, by simply
adding a rule to generate it, like:
